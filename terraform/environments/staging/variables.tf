...
variable "environment" {
  ...
  validation {
    ...
    error_message = "The staging environment stack must use environment = \"staging\"."
  }
}
...
variable "db_skip_final_snapshot" {
  description = "Skip the final snapshot when destroying the staging database."
  type        = bool
  default     = true
}

variable "db_performance_insights_enabled" {
  description = "Enable Performance Insights for the staging database."
  type        = bool
  default     = true
}
...
variable "redis_num_cache_clusters" {
  description = "Number of Redis nodes in staging."
  type        = number
  default     = 1
}
